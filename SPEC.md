...
- **G3**: タスクから直接 Claude Code / Codex / Antigravity を該当 repo で起動可能
...
| `source_type` | TEXT | `code_todo` \| `github_issue` \| `github_pr` \| `gh_notification` \| `gh_run_failure` \| `gh_project_card` \| `git_interrupted` \| `git_stash` \| `orphan_branch` \| `claude_session_todo` \| `codex_session_todo` \| `antigravity_session_todo` \| `cursor_session_todo` \| `agents_note` \| `manual` |
...
| `assignee` | TEXT | `claude-code` \| `codex` \| `antigravity` \| `self` \| `human-review` |
...
| `session_type` | TEXT NULL — `claude` / `codex` / `antigravity` / `cursor` |
| `created_at` | TEXT |
...
- lookback: `[claude_session].lookback_days` (default **7**) 以内に更新された全 jsonl を対象とする (codex/antigravity と同等)。これにより同プロジェクトの複数 session が漏れなく ingest される。
- source_id:
...
- repo 解決:
  - chat directory 名は `md5(absolute project cwd)` の hex。`scan.roots`
    ...
    旧 `gemini-session` の sha256(cwd) → projects.json 逆引きと同じ pattern (Antigravity CLI は `~/.gemini/antigravity-cli/history.jsonl` に workspace 絶対パスを直接保持するためハッシュ不要)。
...
- assignee: `self` 固定。Cursor は IDE 内で起動するエージェントなので
  relay 側から `claude-code` / `codex` / `antigravity` のように再起動できる
  単一プロセス・パスを持たない。次の action は人間判断。
- priority:
  - plan todo: 55 (`codex_session` / `antigravity_session` と同 band)。
...
- lookback: `[cursor_session].lookback_days` (default **14**、codex/antigravity の 7
...
| `relay doctor [--strict]` | `rg / gh / claude / codex / agy / git` 検出 |
...
| `relay context save [--auto] [--repo X] [--summary ...] [--session-type claude\|codex\|antigravity\|cursor]` | repo snapshot。`--auto` は Claude Stop hook 用 (stdin から payload) |
...
4. assignee に応じて launcher を選択:
   ...
   - `codex`: `cd <repo> && codex resume <session_id>` または `cd <repo> && codex '<prompt>'`
   - `antigravity`: `cd <repo> && agy '<prompt>'`
   - `self`: editor 起動 (`$EDITOR <repo>`)
   - `human-review`: GitHub PR URL を `open`
5. launcher プロセス終了時、`runs.ended_at` を更新
6. Codex / Antigravity は launcher 終了時に context snapshot を保存する。Claude Code は Stop hook で保存する。

### 10.1 Repo-scoped prompt template (`.agents/RELAY_PROMPT.md`)

リポごとに固定の前置きを `<repo>/.agents/RELAY_PROMPT.md` に置くと、`claude-code` / `codex` / `antigravity` 起動時の prompt 先頭に自動挿入される。プロンプト合成順は **`repo template` → `context preamble` → `task.prompt`** (各セクション間は `---` 区切り)。
...
### Save (session 終了時 hook / relay run completion)
...
- `session_type='claude'` として `contexts` テーブルに insert し、関連 task の `context_hash` を更新

Codex / Antigravity は汎用 Stop hook を持たないため、`relay run` のプロセス終了時に `session_type='codex'` / `session_type='antigravity'` の snapshot を保存する。また `relay sync` の `codex_session_todo` / `antigravity_session_todo` adapter は session の `cwd` が tracked repo に解決できた場合に context snapshot を保存し、取り込んだ task の `context_hash` に同じ hash を設定する。手動保存では `--session-type` で family を指定できる。
...
- `claude-code` / `codex` は `session_id` があり `--no-resume` が無い場合、既存 session を resume するため preamble / template を注入しない。
- `--no-resume` を付けると preamble は注入される。Antigravity は UUID resume 未対応のため常に preamble fallback で動作する。
...
| **v1.0** | export/import、複数 root 対応の最適化、Codex/Antigravity launcher |

## 15. Open Questions

- Q1: Codex CLI の session resume API は仕様確定 (`codex resume <id>`)。Antigravity CLI (`agy`) は UUID resume 未対応のため preamble fallback で運用中。
