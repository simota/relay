// Port of src/tui/fuzzy.ts — fzf-style scorer with multi-term AND.
...
export function fuzzyMatchMulti(query: string, candidate: string): FuzzyMatch | null {
...
>(tasks: T[], query: string): Filtered<T>[] {
  if (!query.trim()) return tasks.map((task) => ({ task, score: 0, titleIndices: [] }));
  const out: Filtered<T>[] = [];
